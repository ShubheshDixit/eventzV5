...
for item in data:
    ...
    if (_date - latest_date).days >= 0:
        # print((_date - latest_date).seconds//3600)
        print(data[item]['event'])
        # if not data[item]['image_url'] == None or not data[item]['image'] == None:
        #     for event in new_events:
        #         if event['image_url'] == data[item]['image_url']:
        #             break
        #     else:
                # if data[item]['behavior_set'] == "3":
                #     new_series.append(data[item])
                # else:
        for event in new_events:
            if event['image_url'] == data[item]['image_url']:
                break
...
data = {'events': new_events,'series': new_series}
with open('new_events.json', 'w') as f:
    data = json.dump(data, f, indent=4)
print(len(new_events), len(new_series))
# ipdb.sset_trace()